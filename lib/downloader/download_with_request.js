...
let userAgents = require(path.join(__dirname,'../util/userAgents')).userAgents
...
var exec = async function(stream, opts){
			...
			if(!opts.onlyMerge){
				...
				queue.add(() => {
					try{
						...
						logger.info("debug: downloading err " +err)
					}
				});
			}else{
				success = true
	...
	logger.info("debug: downloaded result " + success)
