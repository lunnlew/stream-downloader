...
var exec = async function(opts){
		...
		result = await new Promise((resolve, reject) => {
			...
			try{
				...
				cmd_child.stderr.on('data', function (data) {
					logger.info("debug: err " + iconv.decode(data, 'gb2312').toString())
					resolve("")
				});
			}catch(error){
				logger.info("debug: err " + error)
				reject(error)
			}
		  }
		)
	...
	if(fs.pathExistsSync(result)){
		...
		logger.info("debug: local not exists aria2c ")
	}
	...
	if(os_platform=='win32'){
		...
		try{
			logger.info("debug: starting download aria2c ")
			...
			logger.info("debug: download result " + ffresult.state)
			if(ffresult.state == "success"){
				logger.info("debug: unzip aria2c.zip ")
				...
				await new Promise((resolve, reject) => {
					try{
						...
						logger.info("debug: move aria2c error " + err)
					}
				})
				logger.info("debug: clear tempfile ")
				try{
					await Promise.all([path.normalize(`aria2-${version}-${os_platform}-${os_arch}-build1`)].map(unlink))
				}catch(err){
					logger.info("debug: rm tempfile file failed")
				}
			...
			logger.info("debug: download aria2c error " + err)
		}
	} else {
		logger.info(`debug: current platform(${os_platform}-${os_arch}) not support downloading aria2c to install`)
	}
	return result
}
