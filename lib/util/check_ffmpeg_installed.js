...
var exec = async function(opts) {
        ...
        if (os_platform == 'win32') {
            ...
            result = await new Promise((resolve, reject) => {
                ...
                try {
                    ...
                    cmd_child.stderr.on('data', function(data) {
                        logger.info("err " + iconv.decode(data, 'utf-8').toString())
                        resolve("")
                    });
                } catch (error) {
                    logger.info("err " + error)
    ...
    if (os_platform == 'win32') {
        ...
        version = '20191006'
        sourceUrl = `https://johnvansickle.com/ffmpeg/builds/ffmpeg-git-${os_platform}-${lib_link}.tar.xz`
        outFile = `ffmpeg-git-${os_platform}-${lib_link}.tar.xz`
        zipExt = 'tar.xz'
        sourceDir = `ffmpeg-git-${version}-${os_platform}-${lib_link}`
        sourceExe = `ffmpeg-git-${version}-${os_platform}-${lib_link}/ffmpeg`
        destExe = 'bin/ffmpeg'
    } else {
