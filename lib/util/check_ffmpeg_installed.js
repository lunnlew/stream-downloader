...
var exec = async function(opts){
	...
	if(fs.pathExistsSync(result)){
		...
		logger.info("debug: local not exists ffmpeg ")
	}
	...
	if(os_platform=='win32'){
		...
		try{
			logger.info("debug: starting download ffmpeg ")
			...
			if(ffresult.state == "success"){
				logger.info("debug: unzip ffmpeg.zip ")
			...
			logger.info("debug: download ffmpeg error " + err)
		}
	} else {
		logger.info(`debug: current platform(${os_platform}-${os_arch}) not support downloading ffmpeg to install`)
	}
	return result
}
