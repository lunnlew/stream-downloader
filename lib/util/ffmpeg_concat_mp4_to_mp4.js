	...
	if(files.length){
	  	...
	  	for(let i in files){
	  		...
	  		queue.add(() => {
	  			return new Promise((resolve, reject) => {
				  	logger.info('debug: start covert with ' + file)
				    ...
				    try{
					    ffmpeg(file)
							...
							.on('start', function(commandLine) {
								logger.info('debug: Ffmpeg with command: ' + commandLine)
							})
							.on('end', () => {
								names[i] = outfile
								resolve(outfile)
						...
						logger.info('debug: ffmpeg covert err '+err)
						resolve()
					}
		  		})
	  		})
		...
		await new Promise((resolve, reject) => {
				try{
					...
					logger.info('debug: ffmpeg merge err '+err)
					resolve()
				}
			})
...
var exec = async function(tempfiles, outputpath, outputfilename, opts){
	...
	logger.info("debug: ffmpegPath " + ffmpegPath)

	ffmpeg.setFfmpegPath(ffmpegPath)

	logger.info('debug: staring merge video')
	try{
		await ffmpeg_concat(tempfiles, outputpath, outputfilename, opts)
		logger.info('debug: merge completed')
		return {
			'state':'success',
			'outputfile':path.join(outputpath, outputfilename)
		}
	}catch(err){
		logger.info('debug: merge error' +err)
		return {
			'state':'fail',
			'err':err
		}
