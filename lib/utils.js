...
var urlMatchToParser = function(params, opts){
    ...
    if(fs.pathExistsSync(extractorfile+'.js')){
         logger.info('debug: use extractor '+extractorFullName)
        parser = require(extractorfile)
    }else{
        logger.info('debug: use extractor '+extractorName)
...
var checkMediaSource = async function(url, opts){
    logger.info('debug: start checkMediaSource')
    ...
    logger.info('debug: checkMediaSource type '+type)
    let mediaTypes = [
    'bin', 'exe', 'zip',
    'jpeg', 'jpg', 'png', 'webp',
    'mp3', 'm4a',
