...
var urlMatchToParser = function(params, opts){
    ...
    for (var rule in match_rules){
        if(match_rules[rule].test(params.url)){
            if(typeof opts.extractorNum != 'undefined'){
                let p_parser = path.join(__dirname,'./parser/'+rule+'_'+opts.extractorNum)
                if(fs.pathExistsSync(p_parser)){
                    parser = require(p_parser)
                    break;
                }
            }
