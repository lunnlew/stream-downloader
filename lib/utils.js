                ...
                return new Promise((resolve, reject) => {
                    try{
                        if(fs.pathExistsSync(outputfile)){
                            ...
                            fs.move(filepath, path.join(output, outputfile), { overwrite: true }, err => (err ? reject(err) : resolve()))
                        }
                    }catch(err){
                        reject(err)
                    }
                ...
                return new Promise((resolve, reject) => {
                    try{
                        if(fs.pathExistsSync(outputfile)){
                            ...
                            fs.move(filepath, path.join(output, i+'_'+outputfile), { overwrite: true }, err => (err ? reject(err) : resolve()))
                        }
                    }catch(err){
                        reject(err)
                    }
...
var urlMatchToParser = function(params, opts){
    ...
    if(fs.pathExistsSync(extractorfile+'.js')){
         console.log('debug: use extractor '+extractorFullName)
        parser = require(extractorfile)
    }else{
        console.log('debug: use extractor '+extractorName)
...
var checkMediaSource = async function(url, opts){
    ...
    let content = await request.agent({'referer':url}, opts).head(url).proxy(opts['httpProxy']).catch(err => {
        let headers = {}
        if (err.status === 301) {
          return request.agent({'referer':url}, opts).head(err.response.headers['location'])
