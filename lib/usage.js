...
global.silent = process.argv.indexOf('-O')
global.logger = getLogger()

var optsParser = function (argv, pkginfo) {
    logger.info(`${pkginfo.name} - ${pkginfo.description}`);
    ...
    if (opts.version) {
        logger.debug((opts.verbose ? pkginfo.name + ' v' : '') + pkginfo.version);
        process.exit();
    }


    if (opts.help || opts._.length === 0) {
        logger.debug(_optsParser.generateHelp());
    }
