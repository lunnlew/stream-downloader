...
const {requestStrToMap, md5} = require(path.join(__dirname,'../utils'))
...
var exec = async function(params, opts){
	...
	let content_s = await request.agent({'url':params.url}, opts)
        .post(info_url)
        .set('referer', params.url)
    try{
        let json=JSON.parse(content_s.text)
    }catch(err){
        console.log('debug: fetch info error!')
        return []
    }
	...
	for(let rate in vd_2_id){
		...
		let content = await request.agent({'url':params.url}, opts)
	        .post(info_api)
	        .set('referer', params.url)
	        .type('form')
	        .send(payload)
	    ...
	    if(vjson['error']==0){
	    	...
	    	let matched = vjson['data']['rtmp_live'].split('?')[0].match(/([^\.\/\\]+)\.([a-z]+)$/i)
    		let ext = matched[2]
	    	streams[stream_id] = {
				'id':stream_id,
