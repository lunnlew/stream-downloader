...
var exec = async function(params, opts){
    logger.info("debug: matching audio info ")
    ...
    logger.info("debug: hash " + hash)

    let album_id = url_macth[3]
    logger.info("debug: album_id " + album_id)
 ...
 //    let album_id_match = body.match(/"album_id":([A-Z0-9]+)/iu);
    ...
    logger.info("debug: song_name " + song_name)
    ...
    logger.info('debug: info_url '+info_url)
    ...
    logger.info("debug: matching audio completeed ")
    return [{
        "title":song_name,
        "url":params.url,
