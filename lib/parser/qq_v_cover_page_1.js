...
var exec = async function(params, opts){
	...
	try{
		...
		let cookies_s = cookies_str.split("; ")
		let cookies = []
		cookies_s.forEach(function(item){
			let c = item.split("=")
				cookies.push({
			    key: c[0],
			    value: c[1],
	    ...
	    for (let part_format_id in vd_2_id) {
		    ...
		    let content = await request.get(url,false,{
				"referer":"https://vv.video.qq.com",
				"User-Agent":UserAgent,
			},{
				"cookies":cookies,
				"url":"https://vv.video.qq.com"
			})
		    ...
		    for (let i = 1; i < seg_cnt+1; i++) {
		        ...
		        let part_info = await request.get(key_api,false,{
					"referer":"https://vv.video.qq.com",
					"User-Agent":UserAgent,
				},{
					"cookies":cookies,
