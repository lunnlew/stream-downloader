	...
	try{
		 ...
		 for (let part_format_id in vd_2_id) {
			...
			if(video_json.errCode==0){
				...
				// 视频是否可下载
			    if(video_json['s']=="f"){
			    	logger.info("debug: video "+video_json['msg'])
				    can_download = false
			    }else{
			    	can_download = true
			    }
			    if(can_download){
					...
					if(video_json['vl']['vi'][0]['ul']['m3u8']){
						...
						logger.info("debug: has hls")
						let pt = video_json['vl']['vi'][0]['ul']['ui'][0]['hls']['pt']
						if(pt==''){
							logger.info("debug: not valid source")
							continue
						}
						let vurl = `${host}${pt}`
						logger.info("debug: vurl " + vurl)
