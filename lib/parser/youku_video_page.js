...
var exec = async function(params, opts){
	...
	let cna = cna_macth[1]
	logger.info("debug cna " + cna)

	await sleep(1000)

	let ccode = "0502"
	logger.info("debug ccode " + ccode)
	...
	if(vjson['data']['data']['error']){
		logger.info("debug: "+vjson['data']['data']['error']['note'])
		return []
	}
	...
	for(let i in vdata['stream']){
		...
		// 由于加密因素，这里暂不使用m3u8
		// _getDecodeKey
		// see e.decodeKey && a && (f.data = et(f.data, e.decodeKey))
		if(false && stream['m3u8_url']){
	...
	var streams_sorted = [];
	for(let stype of stream_types){
		let stream_id = stype['id']
		if (stream_id in streams){
