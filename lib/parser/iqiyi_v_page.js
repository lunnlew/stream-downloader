...
var exec = async function(params, opts) {
    ...
    for (let bid of ids) {
        ...
        if (data['data']['boss_ts'] && data['data']['boss_ts']['code'] != 'A00000') {
            logger.info('boss_ts ' + data['data']['boss_ts']['msg'])
            continue
        }
        if (!data['data']['program']) {
            logger.warn('需要客户端方式的解析')
            break
        }
        ...
        for (let video of videos) {
            ...
            logger.info('video ff ' + video['ff'])
            if (video['drmType']) {
                logger.info('video drmType ' + video['drmType'])
            }
            if (video['drmType'] == 7) {
                logger.warn('not support parse track')
                break
            }
            // 有m3u8数据
