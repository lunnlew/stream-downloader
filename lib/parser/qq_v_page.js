...
var exec = async function(params, opts){
	...
	try{
	    ...
	    for (let part_format_id in vd_2_id) {
		    ...
		    // 视频是否可下载
		    if(video_json['s']=="f"){
		    	logger.info("debug: "+video_json['msg'])
			    can_download = false
		    	break
		    }
		    ...
		    for (let i = 1; i < seg_cnt+1; i++) {
		        ...
		        let part_info = await agent.get(key_api).proxy(opts['httpProxy'])
			    ...
			    // 视频清晰度是否可下载
			    if(part_json['s']=="f"){
			    	logger.info("debug: "+part_json['msg'])
	...
	if(!can_download){
		logger.info("debug: video cannot download ")
		return []
	}
