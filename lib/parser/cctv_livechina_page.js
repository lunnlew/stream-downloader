...
const {getUrlExt, streamsSort, requestStrToMap} = require(path.join(__dirname,'../utils'))
const ids = ['hls', 'flv', 'hds']
const id_2_profile = {'hls':'hls', 'flv':'flv', 'hds':'hds'}
...
var exec = async function(params, opts){
    ...
    let reqmap = requestStrToMap(params.url.split('?')[1])
    let liveid = reqmap['id']
    logger.info("debug: liveid " + liveid)
    if(!liveid){
        return []
    }
    let infoapi = `http://api.cntv.cn/list/getCboxLiveMessage?serviceId=cbox&id=${liveid}`
    let bodyinfo = await request.agent({'referer':infoapi}, opts).get(infoapi).proxy(opts['httpProxy'])
    let livejson = bodyinfo.body.data
    let title  = livejson['liveTitle']
    var vdn_tsp =new Date().getTime().toString().slice(0,10);
    var vdn_vnFlash = "1537";
    var staticCheck_Flash = "B4B51E8523157ED8D17ADB76041BCD09";
    var vdn_vnHtml5 = "2049";
    ...
    let streams = {}
    let urls = []
    for(let live of livejson['liveList']){
        let sid = live['liveUrl']
        let vdnUrl = "https://vdn.live.cntv.cn/api2/liveHtml5.do?channel=pa://cctv_p2p_hd" + sid
        vdn_vc = setH5Str((vdn_tsp+vdn_vnFlash+staticCheck_Flash+vdn_uid)).toLocaleUpperCase()
        vdnUrl = vdnUrl+= "&client=flash"+"&tsp="+vdn_tsp + "&vn="+ vdn_vnHtml5 + "&vc="+vdn_vc + "&uid="+vdn_uid + "&wlan="+vdn_wlan
        let body = await request.agent({'referer':vdnUrl}, opts).get(vdnUrl).proxy(opts['httpProxy'])
        let match = body.text.match(/html5VideoData='(.*?)';getH/iu)
        let vjson = JSON.parse(match[1])
        let id = 'hls'
        let url = vjson[id+'_url'][id+'2']
        urls.push(url)
    }
    streams['hls'] = {
        "id":'hls',
