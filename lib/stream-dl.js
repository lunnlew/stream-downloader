    ...
    for(let streams_result of streams_results ){
		...
		if(opts.streamManual){
			...
			if(selected){
				...
				logger.info("debug: manual selected "+stream['id'])
			...
			for(let stream_id in streams){
				...
				stream.title = streams_result.title
				stream.merge = stream.merge||streams_result.merge
				logger.info("debug: auto selected "+stream_id)
				selected_stream['video'] = stream
				break
			}
		}
...
var download_extras_task = async function(title, selected_extras, opts){
	...
	// 等待任务执行完毕
	await queue.onIdle()
	logger.info('debug: extras downloaded')
...
var download_stream_task = async function(selected_stream, opts){
	...
	return new Promise(async (resolve, reject) => {
		...
		logger.info('debug: download result')
		...
		queue.add(() => {
			return new Promise(async (resolve, reject) => {
				if(selected_video_stream && selected_video_stream.isDispatch){
					logger.info('debug: new dispatcher')
				    ...
				    if(selected_extras){
				    	logger.info('debug: start download extras')
		       			// 等待附加任务完成
					...
					if(!onlyDownloadExtras){
						...
						logger.info('debug: video_stream download completed')
						if(selected_audio_stream){
							let audio_result = await download_stream_task(selected_audio_stream, opts)
							logger.info('debug: audio_stream download completed')
							// 合并音视频流
						...
						logger.info('debug: finish_result')
						...
						logger.info('debug: move_result')
						logger.info(move_result)
		       			if(move_result){
			       			if(move_result['state'] == 'success'){
			       				logger.info('debug: clear tmp data')
				       			try{
				       				await Promise.all([path.join(require('os').tmpdir(), md5(selected_video_stream.title))].map(unlink))
				       			}catch(err){
				       				logger.info('debug: '+ err)
				       			}
				       			logger.info('debug: finish file in localtion ['+path.resolve(move_result['output'])+']')
				       		}else{
				       			logger.info('debug: file move err')
				       		}
				       	}
				    }
					resolve()
				}else{
					logger.info('debug: not selected stream')
				}
			})
		})
	}
...
var parse_stream_dispatcher = async function(all_play_urls, opts){
	if(!all_play_urls.length){
		logger.info(`debug: Not play page urls`)
	}
	logger.info(`debug: urls total ${all_play_urls.length}`)
	// 播放器
	if(opts.player || opts.outStream){
		...
		for(let i in all_play_urls){
			...
			queue.add(() => {
				return new Promise(async (resolve, reject) => {
					logger.info('debug: parse url ' + streams_params['url'])
					try{
		    			let parser = urlMatchToParser(streams_params, opts)
		    			if(parser){
			    			if(parser.vp){
...
var exec = async function(opts){
    ...
    if(!opts.disabeConfig){
	    logger.info('debug: try load config from '+configJsonFile)
    ...
    if(!opts._.length){
    	logger.info("debug: please set urls ")
    	process.exit()
    }
    let queue = new PQueue({concurrency: 1})
    ...
    opts._.forEach((url, index) => {
    	...
    	queue.add(() => {
			return new Promise(async (resolve, reject) => {
		    	...
		    	if(source['isMedia']){
		    		logger.info('debug: source is media')
	...
	if(!opts.disabeConfig){
    	logger.info('debug: config saved')
