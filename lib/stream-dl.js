					...
					if(selected_audio_stream){
	       			...
	       			if(finish_result['outputfile']){
	       				if(finish_result['tempfiles']){
			       			await require(path.join(__dirname,'./util/move_files')).exec(finish_result['tempfiles'], finish_result['outputfile'], opts)
	       				}else{
			       			await require(path.join(__dirname,'./util/move_files')).exec([finish_result['outputfile']], finish_result['outputfile'], opts)
	       				}
	       			}	
					resolve()
				})
