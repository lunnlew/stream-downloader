...
const urlMatchToParser = require(path.join(__dirname,'util/urlMatchToParser'));
			...
			queue.add(() => {
				return new Promise((resolve, reject) => {
					console.log('Downloading streaming content with FFmpeg, press ctrl + c to stop recording...')
	               	require(path.join(__dirname,'downloader/download_with_ffmpeg')).exec(selected_stream, opts, resolve, reject)
	            })
			});
			...
			queue.add(() => {
				return new Promise((resolve, reject) => {
					console.log('Downloading streaming content with request, press ctrl + c to stop downloading...')
	               	require(path.join(__dirname,'downloader/download_urls')).exec(selected_stream, opts, resolve, reject)
	            })
			});
		}else{
...
var exec = async function(opts){
	...
	// ffmpegPath = await require(path.join(__dirname,'./util/check_ffmpeg_installed')).exec(ffmpegPath, opts)
	// process.exit()
    ...
    if(!opts._.length){
        ...
        if(answers['playNum']){
        	opts['playNum'] = parseInt(answers['playNum'])
        }
        if(answers['ffmpeg']){
        	opts['ffmpeg'] = answers['ffmpeg']
        }
        if(answers['player']){
