					...
					if(selected_audio_stream){
	       			...
	       			if(finish_result['outputfile']){
	       				if(finish_result['tempfiles']){
			       			await require(path.join(__dirname,'./util/move_files')).exec(result['tempfiles'], result['outputfile'], opts)
	       				}else{
			       			await require(path.join(__dirname,'./util/move_files')).exec([result['outputfile']], result['outputfile'], opts)
	       				}
	       			}	
					resolve()
				})
