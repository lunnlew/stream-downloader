	    ...
	    if(result.success && result.all){
	    	if(!onlyDownload){
	        	// 是否需要合并资源
	       		...
	       		resolve({'state':'sunccss', 'msg':"download sunccss"})
	       	}
	   	}else{
	...
	for(let selected_stream of selected_streams){
		...
		if(selected_video_stream.isDispatch){
			...
			queue.add(() => {
				return new Promise(async (resolve, reject) => {
					// 等待视频流任务完成
					let video_result = await download_stream_task(selected_video_stream, opts)
					...
					if(selected_audio_stream){
						...
						// 合并音视频流
						let concat_result = await require(path.join(__dirname,'./util/ffmpeg_concat_av_to_mp4')).exec(video_result['outputfile'], audio_result['outputfile'], video_result['outputfile'], opts)
						resolve()
					}
					resolve()
				})
