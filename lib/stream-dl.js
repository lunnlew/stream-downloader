...
const {urlMatchToParser, checkMediaSource, md5, getUrlExt, renderSize, renderDuration} = require(path.join(__dirname,'./utils'))
		...
		if(selected_video_stream.isDispatch){
			...
			queue.add(() => {
				return new Promise(async (resolve, reject) => {
	    			...
	    			// }
	       			console.log('debug: file localtion ['+path.resolve(move_result['output'])+']')
					resolve()
				})
			})
		}
