		...
		if(selected_stream['container']=='m3u8'){
			queue.add(() => {
				return new Promise(async (resolve, reject) => {
					...
					// 是否启用了aria2下载
					if(enableAria2c){
						console.log('Downloading streaming content with Aria2c, press ctrl + c to stop recording...')
						// 处理m3u8内容
						let parts = selected_stream['src'][0].split('/')
						parts.pop()
						let prepath = parts.join('/')
						// 下载
						selected_stream['format'] = 'm3u8'
						...
						// 实际下载
						selected_stream['src'] = urls
						selected_stream['filepath'] = filepath
						selected_stream['remote'] = false
