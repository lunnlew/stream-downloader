...
var exec = async function(opts){
    ...
    opts._.forEach((url, index) => {
    	...
    	if(!opts.disabeConfig){
	    	...
	    	if(!opts['httpCookies']){
		    	if(configOpts['httpCookies'] && url_part.hostname in configOpts['httpCookies']){
		    		opts['httpCookies'] = configOpts['httpCookies'][url_part.hostname]
		    	}
		    }else{
		    	if(!('httpCookies' in configOpts)){
		    		configOpts['httpCookies'] = {}
		    	}
		    	configOpts['httpCookies'][url_part.hostname] = opts['httpCookies']
		    }
		}
