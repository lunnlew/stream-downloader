...
const {urlMatchToParser, checkMediaSource, md5, getUrlExt, renderSize, renderDuration, unlink} = require(path.join(__dirname,'./utils'))
...
var exec = async function(opts){
    ...
    if(!opts.disabeConfig){
	    ...
	    opts = Object.assign({}, configOpts, opts)
	    fs.outputJsonSync(configJsonFile, opts)
	}
